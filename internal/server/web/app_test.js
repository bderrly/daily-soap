...
async function loadApp(window) {
  const code = await Deno.readTextFile(new URL("./app.js", import.meta.url));
  // Wrap in a function to pass window as global
  const fn = new Function("window", "document", "Intl", "fetch", "Node", "setTimeout", "clearTimeout", code);
  fn(window, window.document, window.Intl, window.fetch, window.Node, window.setTimeout, window.clearTimeout);
}
